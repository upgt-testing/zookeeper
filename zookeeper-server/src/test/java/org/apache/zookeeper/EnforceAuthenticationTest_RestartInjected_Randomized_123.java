...
public class EnforceAuthenticationTest_RestartInjected_Randomized_123 extends QuorumPeerTestBase {

    protected static final Logger LOG = LoggerFactory.getLogger(EnforceAuthenticationTest_RestartInjected.class);
    ...
    private void testEnforceAuthOldBehaviour(boolean netty) throws Exception {
        ...
        if (netty) {
            config.setProperty(ZKClientConfig.ZOOKEEPER_CLIENT_CNXN_SOCKET, "org.apache.zookeeper.ClientCnxnSocketNetty");
        }
        ZooKeeper client = ClientBase.createZKClient("127.0.0.1:" + clientPort, CONNECTION_TIMEOUT, CONNECTION_TIMEOUT, config);
        ...
        RestartFramework.at("after_client_connected").on(servers.mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_create_node").on(servers.mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        client.close();
    }
    ...
    private void testEnforceAuthNewBehaviour(boolean netty) throws Exception {
        ...
        ZooKeeper client = new ZooKeeper("127.0.0.1:" + clientPort, CONNECTION_TIMEOUT, getWatcher(countDownLatch), config);
        ...
        RestartFramework.at("after_add_auth").on(servers.mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        client.addAuthInfo("digest", idPassword.getBytes());
        client.create(path, data.getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        byte[] data1 = client.getData(path, false, null);
        RestartFramework.at("after_reconnect").on(servers.mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(data, new String(data1));
        RestartFramework.at("after_create_with_auth").on(servers.mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        client.close();
    }
