 ...
 * ZooKeeper
 */
public class RaceConditionTest_RestartInjected_Randomized_123 extends QuorumPeerTestBase {
    ...
    public void testRaceConditionBetweenLeaderAndAckRequestProcessor() throws Exception {
        mt = startQuorum();
        RestartFramework.at("after_wait_leader_state_change").on(mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // get leader
        QuorumPeer leader = getLeader(mt);
        long oldLeaderCurrentEpoch = leader.getCurrentEpoch();
        RestartFramework.at("after_old_leader_up").on(mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_start_quorum").on(mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        assertTrue(ClientBase.waitForServerUp("127.0.0.1:" + leader.getClientPort(), CONNECTION_TIMEOUT), "Failed to bring up the old leader server");
        RestartFramework.at("after_shutdown_followers").on(mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertTrue(leaderStateChanged || (leader.getCurrentEpoch() > oldLeaderCurrentEpoch), "Leader failed to transition to new state. Current state is " + leader.getServerState());
    }
