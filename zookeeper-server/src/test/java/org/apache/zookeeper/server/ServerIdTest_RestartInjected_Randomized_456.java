...
public class ServerIdTest_RestartInjected_Randomized_456 extends ClientBase {
    ...
    public void doTest(boolean ttlsEnabled, int serverId) throws Exception {
        ...
        try {
            ...
            if (ttlsEnabled) {
                ...
                RestartFramework.at("after_create_foo_with_ttl").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            } else {
                try {
                    zk.create("/foo", new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT_WITH_TTL, new Stat(), 1000);
