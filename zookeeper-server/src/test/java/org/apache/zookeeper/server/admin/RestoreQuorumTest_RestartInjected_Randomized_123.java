...
public class RestoreQuorumTest_RestartInjected_Randomized_123 extends QuorumPeerTestBase {
    ...
    public void testRestoreAfterQuorumLost() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_create_parent_node").on(servers.mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            for (int i = 0; i < NODE_COUNT; i++) {
                servers.zk[leaderId].create(PATH + "/" + i, new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
            }
            RestartFramework.at("after_initial_data_creation").on(servers.mt[1]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // take snapshot
            final File snapshotFile = takeSnapshotAndValidate(servers.adminPorts[leaderId], ClientBase.testBaseDir);
            RestartFramework.at("after_snapshot").on(servers.mt[2]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // create more data
            for (int i = NODE_COUNT; i < NODE_COUNT * 2; i++) {
                servers.zk[leaderId].create(PATH + "/" + i, new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
            }
            RestartFramework.at("after_additional_data_creation").on(servers.mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_quorum_restored").on(servers.mt[1]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // restore servers
            for (int i = 0; i < SERVER_COUNT; i++) {
                performRestoreAndValidate(servers.adminPorts[i], snapshotFile);
            }
            RestartFramework.at("after_restore_complete").on(servers.mt[2]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_post_restore_data").on(servers.mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
