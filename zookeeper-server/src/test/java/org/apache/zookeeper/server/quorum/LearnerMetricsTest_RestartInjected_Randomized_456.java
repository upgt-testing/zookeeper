...
public class LearnerMetricsTest_RestartInjected_Randomized_456 extends QuorumPeerTestBase {

    // 1 observer, 3 participants
    ...
    public void testLearnerMetricsTest(boolean asyncSending) throws Exception {
        ...
        RestartFramework.at("after_observer_start").on(mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_participants_start").on(mt[1]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_connect_observer").on(mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_metrics_verification").on(mt[0]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
