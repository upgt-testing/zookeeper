...
public class FourLetterWordsQuorumTest_RestartInjected_Randomized_123 extends QuorumBase {
    ...
    public void testFourLetterWords() throws Exception {
        ...
        for (String hp : servers) {
            ...
            verify(hp, "wchp", "");
            verify(hp, "wchc", "");
            RestartFramework.at("after_initial_verifications").on(this).restart("leader").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_create_client").on(this).restart("follower").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_set_watch").on(this).restart("leader").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            verify(hp, "stat", "queued");
            verify(hp, "srvr", "Outstanding");
            verify(hp, "cons", sid);
            verify(hp, "dump", sid);
            ...
            verify(hp, "wchc", sid);
            verify(hp, "dirs", "size");
            RestartFramework.at("after_watch_verifications").on(this).restart("follower").withIndex(1).withMode(RestartMode.GRACEFUL).execute();
            zk.close();
            RestartFramework.at("after_close_client").on(this).restart("leader").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            verify(hp, "ruok", "imok");
            verify(hp, "envi", "java.version");
            verify(hp, "conf", "clientPort");
            verify(hp, "stat", "Outstanding");
            ...
            RestartFramework.at("after_final_verifications").on(this).restart("follower").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
