...
public class ClientSSLTest_RestartInjected_Randomized_456 extends QuorumPeerTestBase {
    ...
    public void testClientServerSSL_positive(SslProvider sslProvider, String fipsEnabled, String hostnameVerification) throws Exception {
        ...
        // Act & Assert
        testClientServerSSL(hostnameVerification.equals("true") ? "localhost" : InetAddress.getLocalHost().getHostName(), true, CONNECTION_TIMEOUT);
    }
    ...
    public void testClientServerSSL_negative(SslProvider sslProvider, boolean fipsEnabled) {
        ...
        // Act & Assert
        assertThrows(AssertionError.class, () -> testClientServerSSL(InetAddress.getLocalHost().getHostName(), true, 5000));
    }
    ...
    public void testClientServerSSL(String hostname, boolean useSecurePort, long connectTimeout) throws Exception {
        ...
        for (int i = 0; i < SERVER_COUNT; i++) {
            assertTrue(ClientBase.waitForServerUp("127.0.0.1:" + clientPorts[i], TIMEOUT), "waiting for server " + i + " being up");
            ...
            try (ZooKeeper zk = ClientBase.createZKClient(hostname + ":" + port, TIMEOUT, connectTimeout)) {
                ...
                RestartFramework.at("after_create_ssl_test_node").on(mt[i]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                zk.delete("/test", -1);
                RestartFramework.at("after_delete_ssl_test_node").on(mt[i]).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    ...
    public void testSecureStandaloneServer() throws Exception {
        ...
        zk.create("/test", "".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        RestartFramework.at("after_delete_standalone_test_node").on(mt).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        zk.delete("/test", -1);
        RestartFramework.at("after_create_standalone_test_node").on(mt).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        zk.close();
        RestartFramework.at("after_standalone_server_started").on(mt).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        mt.shutdown();
    }
    ...
    public void testSecureStandaloneServerAuthFail() throws IOException {
        try {
            System.setProperty(ProviderRegistry.AUTHPROVIDER_PROPERTY_PREFIX + "authfail", AuthFailX509AuthenticationProvider.class.getName());
            ...
            assertThat("Exception message does not match (different exception caught?)", ex.getMessage(), startsWith("ZooKeeper client can not connect to"));
        } finally {
