...
public class SessionInvalidationTest_RestartInjected_Randomized_456 extends ClientBase {
    ...
    public void testCreateAfterCloseShouldFail() throws Exception {
        RestartFramework.at("after_socket_operations").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        for (int i = 0; i < 10; i++) {
        ...
        RestartFramework.at("after_verify_children").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
