...
public class GetChildren2Test_RestartInjected_Randomized_456 extends ClientBase {
    ...
    public void testChild() throws IOException, KeeperException, InterruptedException {
        ...
        assertEquals(stat.getCzxid(), stat.getMzxid());
        RestartFramework.at("after_get_children_parent").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(stat.getCzxid(), stat.getPzxid());
        assertEquals(stat.getCtime(), stat.getMtime());
        assertEquals(0, stat.getCversion());
        RestartFramework.at("after_get_children_child").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        assertEquals(0, stat.getVersion());
        assertEquals(0, stat.getAversion());
    ...
    public void testChildren() throws IOException, KeeperException, InterruptedException {
        ...
        RestartFramework.at("after_create_parent_node").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_final_get_children").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        List<String> c_b = p;
