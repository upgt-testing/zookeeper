...
public class SaslAuthFailTest_RestartInjected extends SaslAuthDigestTestBase {
    ...
    public void testBadSaslAuthNotifiesWatch() throws Exception {
        try (ZooKeeper ignored = createClient(new MyWatcher(), hostPort)) {
            RestartFramework.at("after_client_create_with_watcher")
                .on(this)
                .restart("server")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
            ...
            authFailed.await();
            RestartFramework.at("after_auth_failed_event")
                .on(this)
                .restart("server")
                .withIndex(0)
                .withMode(RestartMode.GRACEFUL)
