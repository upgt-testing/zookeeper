...
public class UnsupportedAddWatcherTest_RestartInjected_Randomized_123 extends ClientBase {
    ...
    public void testBehavior() throws IOException, InterruptedException, KeeperException {
        assertThrows(KeeperException.MarshallingErrorException.class, () -> {
            try (ZooKeeper zk = createClient(hostPort)) {
                ...
                RestartFramework.at("after_create_node").on(UnsupportedAddWatcherTest_RestartInjected.this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
