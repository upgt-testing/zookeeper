...
public class ObserverTest_RestartInjected_Randomized_123 extends QuorumPeerTestBase implements Watcher {

    protected static final Logger LOG = LoggerFactory.getLogger(ObserverTest_RestartInjected.class);
    ...
    public void testObserverOnly() throws Exception {
        ...
        q1.start();
        RestartFramework.at("after_observer_only_start").on(q1).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testObserverWithStandlone() throws Exception {
        ...
        MainThread q1 = new MainThread(1, CLIENT_PORT_QP1, quorumCfgSection);
        RestartFramework.at("after_observer_standalone_start").on(q1).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        q1.start();
        q1.join(ClientBase.CONNECTION_TIMEOUT);
        assertFalse(q1.isAlive());
    }
