...
public class PersistentRecursiveWatcherTest_RestartInjected_Randomized_456 extends ClientBase {

    private static final Logger LOG = LoggerFactory.getLogger(PersistentRecursiveWatcherTest_RestartInjected.class);
    ...
    public void testBasicAsync() throws IOException, InterruptedException, KeeperException {
        try (ZooKeeper zk = createClient(new CountdownWatcher(), hostPort)) {
            ...
            RestartFramework.at("after_add_watch_async").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            internalTestBasic(zk);
        }
    }
    ...
    public void testRemoval() throws IOException, InterruptedException, KeeperException {
        try (ZooKeeper zk = createClient(new CountdownWatcher(), hostPort)) {
            ...
            RestartFramework.at("after_remove_watches").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNoChildEvents() throws Exception {
        try (ZooKeeper zk = createClient()) {
            ...
            RestartFramework.at("after_add_root_watch").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testDisconnect() throws Exception {
        try (ZooKeeper zk = createClient(new CountdownWatcher(), hostPort)) {
            zk.addWatch("/a/b", persistentWatcher, PERSISTENT_RECURSIVE);
            RestartFramework.at("after_add_watch_before_disconnect").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            stopServer();
    ...
    public void testMultiClient() throws IOException, InterruptedException, KeeperException {
        try (ZooKeeper zk1 = createClient(new CountdownWatcher(), hostPort);
            ...
            RestartFramework.at("after_add_watch_multi_client").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSamePathWithDifferentWatchModes() throws Exception {
        try (ZooKeeper zk = createClient()) {
            ...
            RestartFramework.at("after_add_multiple_watches").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Stat stat = new Stat();
    ...
    public void testRootWatcher() throws IOException, InterruptedException, KeeperException {
        try (ZooKeeper zk = createClient(new CountdownWatcher(), hostPort)) {
            zk.addWatch("/", persistentWatcher, PERSISTENT_RECURSIVE);
            RestartFramework.at("after_add_root_watcher").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Stat stat = new Stat();
            zk.create("/a", new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT, stat);
    ...
    private void assertEvent(BlockingQueue<WatchedEvent> events, EventType eventType, String path, Stat stat) throws InterruptedException {
        assertEvent(events, eventType, path, stat.getMzxid());
    }

    private void assertEvent(BlockingQueue<WatchedEvent> events, EventType eventType, String path, long zxid) throws InterruptedException {
        assertEvent(events, eventType, KeeperState.SyncConnected, path, zxid);
    }

    private void assertEvent(BlockingQueue<WatchedEvent> events, EventType eventType, KeeperState keeperState, String path, long zxid) throws InterruptedException {
