...
public class ClientSSLTest_RestartInjected extends QuorumPeerTestBase {
    ...
    public void testClientServerSSL(String hostname, boolean useSecurePort, long connectTimeout) throws Exception {
        ...
        for (int i = 0; i < SERVER_COUNT; i++) {
            ...
            try (ZooKeeper zk = ClientBase.createZKClient(hostname + ":" + port, TIMEOUT, connectTimeout)) {
                ...
                RestartFramework.at("after_create_ssl_test_node")
                    .on(mt[i])
                    .restart("server")
                    .withIndex(0)
                    .withMode(RestartMode.GRACEFUL)
                    .execute();
                zk.delete("/test", -1);
                RestartFramework.at("after_delete_ssl_test_node")
                    .on(mt[i])
                    .restart("server")
                    .withIndex(0)
                    .withMode(RestartMode.GRACEFUL)
    ...
    public void testSecureStandaloneServer() throws Exception {
        ...
        RestartFramework.at("after_standalone_server_started")
            .on(mt)
            .restart("server")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
        ...
        zk.create("/test", "".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        RestartFramework.at("after_create_standalone_test_node")
            .on(mt)
            .restart("server")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
            .execute();
        zk.delete("/test", -1);
        RestartFramework.at("after_delete_standalone_test_node")
            .on(mt)
            .restart("server")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
