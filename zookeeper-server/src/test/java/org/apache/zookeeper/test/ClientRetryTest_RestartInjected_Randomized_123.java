...
public class ClientRetryTest_RestartInjected_Randomized_123 extends ClientBase {
    ...
    public void testClientRetry() throws IOException, InterruptedException, TimeoutException {
        ...
        try {
            ...
            RestartFramework.at("after_first_client_connected").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            try {
                ...
                RestartFramework.at("after_state_verification").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                cdw1.reset();
                zk.close();
                cdw1.waitForDisconnected(CONNECTION_TIMEOUT);
                RestartFramework.at("after_first_client_disconnected").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                cdw2.waitForConnected(CONNECTION_TIMEOUT);
                RestartFramework.at("after_second_client_connected").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
