...
public class CheckTest_RestartInjected_Randomized_123 extends ClientBase {
    ...
    public void testStandalone() throws Exception {
        ...
        RestartFramework.at("after_test_operations").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    }
    ...
    public void testCluster() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_observer_test").on(qb).restart("observer").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            testOperations(qb.createClient(new CountdownWatcher(), QuorumPeer.ServerState.FOLLOWING));
            RestartFramework.at("after_follower_test").on(qb).restart("follower").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            testOperations(qb.createClient(new CountdownWatcher(), QuorumPeer.ServerState.LEADING));
            RestartFramework.at("after_leader_test").on(qb).restart("leader").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            try {
                qb.tearDown();
    ...
    public void testClusterDatabaseReloadAfterCheck() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_leader_test_operations").on(qb).restart("leader").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            qb.shutdown(leader);
            RestartFramework.at("after_leader_shutdown").on(qb).restart("follower").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            FileTxnSnapLog txnSnapLog = new FileTxnSnapLog(dataDir, dataDir);
            ZKDatabase database = new ZKDatabase(txnSnapLog);
            database.loadDataBase();
            RestartFramework.at("after_database_load").on(qb).restart("follower").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            try {
                qb.tearDown();
            } catch (Exception ignored) {
