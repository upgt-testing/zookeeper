...
public class NullDataTest_RestartInjected_Randomized_123 extends ClientBase implements StatCallback {
    ...
    public void testNullData() throws IOException, InterruptedException, KeeperException {
        ...
        try {
            ...
            RestartFramework.at("after_create_null_data").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            zk.exists(path, false);
            RestartFramework.at("after_sync_exists").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
