...
public class ClientHammerTest_RestartInjected_Randomized_456 extends ClientBase {

    protected static final Logger LOG = LoggerFactory.getLogger(ClientHammerTest_RestartInjected.class);
    ...
    private static class SuperHammerThread extends HammerThread {
        ...
        private final ClientHammerTest_RestartInjected parent;

        private final String prefix;

        SuperHammerThread(String name, ClientHammerTest_RestartInjected parent, String prefix, int count) {
            super(name, count);
            this.parent = parent;
            this.prefix = prefix;
        }
    ...
    public void runHammer(final int threadCount, final int childCount) throws Throwable {
        try {
            ...
            RestartFramework.at("after_start_all_threads").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            verifyHammer(start, threads, childCount);
            RestartFramework.at("after_verify_hammer").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } catch (Throwable t) {
    ...
    public void testHammerSuper() throws Throwable {
        try {
            ...
            RestartFramework.at("after_create_roots_and_start_threads").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            verifyHammer(start, threads, childCount);
            RestartFramework.at("after_verify_super_hammer").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } catch (Throwable t) {
            LOG.error("test failed", t);
            throw t;
        }
    ...
    public void verifyHammer(long start, HammerThread[] threads, int childCount) throws IOException, InterruptedException, KeeperException {
        ...
        for (HammerThread h : threads) {
            ...
            verifyThreadTerminated(h, (long) threads.length * (long) childCount * HAMMERTHREAD_LATENCY * (long) safetyFactor);
        }
