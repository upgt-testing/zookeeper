...
public class SaslAuthRequiredTest_RestartInjected_Randomized_123 extends SaslAuthDigestTestBase {
    ...
    public void testClientOpWithValidSASLAuth() throws Exception {
        ...
        try {
            ...
            RestartFramework.at("after_sasl_client_create").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            zk.create("/foobar", null, Ids.CREATOR_ALL_ACL, CreateMode.PERSISTENT);
            RestartFramework.at("after_create_with_sasl_acl").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
