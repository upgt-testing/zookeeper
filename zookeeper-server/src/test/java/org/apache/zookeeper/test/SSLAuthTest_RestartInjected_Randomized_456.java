...
public class SSLAuthTest_RestartInjected_Randomized_456 extends ClientBase {
    ...
    public void testRejection() throws Exception {
        ...
        RestartFramework.at("after_untrusted_client_create").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_set_untrusted_keystore").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testMisconfiguration() throws Exception {
        ...
        System.clearProperty(clientX509Util.getSslTruststoreLocationProperty());
        System.clearProperty(clientX509Util.getSslTruststorePasswdProperty());
        RestartFramework.at("after_misconfigured_client_create").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_clear_ssl_properties").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        CountdownWatcher watcher = new CountdownWatcher();
        new TestableZooKeeper(hostPort, CONNECTION_TIMEOUT, watcher);
