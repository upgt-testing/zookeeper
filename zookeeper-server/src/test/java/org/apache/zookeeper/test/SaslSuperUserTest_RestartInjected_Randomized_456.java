...
public class SaslSuperUserTest_RestartInjected_Randomized_456 extends SaslAuthDigestTestBase {
    ...
    public void testSuperIsSuper() throws Exception {
        ...
        RestartFramework.at("after_sasl_super_ops").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testOtherSuperIsSuper() throws Exception {
        ...
        try {
            connectAndPerformSuperOps();
            RestartFramework.at("after_other_sasl_super_ops").on(this).restart("server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            //If the test fails it will most likely fail with a NoAuth exception before it ever gets to this assertion
            assertEquals(authFailed.get(), 0);
        } finally {
